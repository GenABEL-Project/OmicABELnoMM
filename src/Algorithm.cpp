    ...
    for (int i = 0; i < a_iters; i++)
    {
        ...
        #pragma omp parallel default(shared)
        {
             #pragma omp sections
             {
                   ...
                   #pragma omp section
                   {
                        Qr = (type_precision*)malloc(n * r * a_block_size *
                                                     sizeof(type_precision));
                   }
                   #pragma omp section
                   {
        ...
        for (int j = 0; j < y_iters; j++)
        {
            ...
            if (i == 0)
            {
                ...
                matlab_print_matrix("Ypre", n, y_block_size, Y);
                 ...
                 //! qy_top = QL'*Y
                cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                            l, y_block_size, n, 1.0, QL, n, Y, n, 0.0,
                            &RQy_top[qy_idx], l);

                matlab_print_matrix("Ypost", n, y_block_size, Y);

                matlab_print_matrix("QL'*Y", l, y_block_size,
                                    &RQy_top[qy_idx]);

                //! K | RtlQlY =RTL\qy_top
                cblas_strsm(CblasColMajor, CblasLeft, CblasUpper,
                            CblasNoTrans, CblasNonUnit,
                            l, y_block_size, 1.0, Rtl, l,
                            &RQy_top[qy_idx], l);
                qy_idx += y_block_size*l;
            }
            get_ticks(end_tick);
            ...
            //! qy_bot = Qr'*Y
            cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                        r * a_block_size, y_block_size, n, 1.0,
                        Qr, n, Y, n, 0.0, Qy_bot, r * a_block_size);
            // matlab_print_matrix("Qr'*Y", r*a_block_size,
            // y_block_size, Qy_bot);
            ...
            #pragma omp parallel default(shared)
            {
                ...
                for (int ii = 0; ii<a_block_size; ii++)
                {
                    ...
                    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                l, y_block_size, r, -1.0, &RtlRtr[ii*l*r],
                                l, &B_bot[b_bot_idx], r, 1.0,
                                &B_top[b_top_idx], l);
...
//                    matlab_print_matrix("bot", r, y_block_size,&B_bot[b_bot_idx]);
                    if (OUTPUT)
                    {
                        prepare_Bfinal(&Bfinal[b_final_idx],
                                       &B_top[b_top_idx],
                                       &B_bot[b_bot_idx],
                                       p, y_block_size, r);
                        AIOfile.write_B(&Bfinal[b_final_idx],
                                        p, y_block_size);
                    }
                    ...
                    if (ForceCheck)
                    {
                        #pragma omp critical
                        {
                            ...
                            prepare_Bfinal(&Bfinal[b_final_idx],
                                           ...
                                           &B_bot[b_bot_idx],
                                           p, y_block_size, r);
                            // matlab_print_matrix("outsol", p,
                            // y_block_size,&Bfinal[b_final_idx]);
                            check_result(AL, &Ar[ii*n*r], n, p,
                                         y_block_size, r, Y,
                                         &Bfinal[b_final_idx]);
                        }
                    }
